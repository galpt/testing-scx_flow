...
#
# Re-assert scx_flow as the configured sched_ext scheduler and restart scx.service.
#
# Usage: sudo sh enable_scx_flow.sh [options]
#
# Options:
#   --flags "..."   Scheduler flags (default: none)
...
require_installation() {
    if [ ! -x "$BINARY_PATH" ]; then
        log_error "${BINARY_PATH} not found. Install scx_flow first."
        exit 1
    fi
...
# Managed by scx_flow enable helper.
...
main() {
    ...
    configure_scx_defaults
    enable_scx_service
    if show_active_scheduler; then
        :
    else
        log_step "Done"
        log_warn "scx.service was restarted, but ${BINARY_NAME} is not the active sched_ext scheduler"
        exit 1
    fi

    log_step "Done"
    log_ok "scx_flow is configured as the primary scheduler for scx.service"
}
