...
#
# Compare schedulers using perf bench sched messaging throughput + cache metrics.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BENCHMARK_SCRIPT="$SCRIPT_DIR/fork_thread_benchmark.sh"
PLOTTER_SCRIPT="$SCRIPT_DIR/fork_thread_benchmarker_plot.py"
RESET_SCRIPT="$SCRIPT_DIR/reset_sched_ext_state.sh"
RESULTS_ROOT="$SCRIPT_DIR/fork-thread-comparison-results"
...
say()  { printf "${BOLD}${CYAN}[fork-thread]${NC} %s\n" "$1"; }
...
usage() {
    cat <<EOF
Usage: ./fork_thread_benchmarker.sh [options]
...
Options:
  ...
  --groups N                perf bench groups (default: ${MSG_GROUPS})
  --nr-loops N              perf bench loops (default: ${NR_LOOPS})
...
FORK_THREAD_GROUPS=
FORK_THREAD_NR_LOOPS=
FORK_THREAD_USE_PIPE=
FORK_THREAD_USE_THREAD=
FORK_THREAD_TIME_SEC=
FORK_THREAD_INSTRUCTIONS=
FORK_THREAD_CYCLES=
FORK_THREAD_IPC=
FORK_THREAD_CACHE_MISSES=
FORK_THREAD_CACHE_REFERENCES=
LOG_PATH=
PERF_STDOUT_PATH=
PERF_STAT_PATH=
...
run_single_benchmark() {
    ...
    say "Running fork/thread benchmark for $label"
...
prune_old_results() {
    ...
    warn "Pruning old fork-thread result directories, keeping the newest ${KEEP_RESULTS}"
...
require_prereqs() {
    [ -x "$BENCHMARK_SCRIPT" ] || { err "Missing fork/thread benchmark script: $BENCHMARK_SCRIPT"; exit 1; }
    ...
    command -v perf >/dev/null 2>&1 || { err "perf is required for fork/thread benchmarking."; exit 1; }
...
render_outputs
prune_old_results
ok "Fork/thread comparison complete: $RESULTS_DIR/tagged"
