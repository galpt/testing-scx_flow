...
#
# Install scx_flow v2.3.2 (Temporal Budget + Adaptive Slice + Lockstep Fix)
# from the galpt/scx fork branch scx_flow-v2.3.2.
#
# v2.3.2 adds the temporal bucket lockstep fix (IDEAS-004): when a task
...
#
# Usage:
#   sudo ./install_scx_flow_v2.3.2.sh
#
# To switch between versions:
#   sudo ./install_scx_flow_v2.3.0.sh    # temporal budget (no adaptive slice)
#   sudo ./install_scx_flow_v2.3.2.sh    # temporal budget + adaptive slice
...
BUILD_DIR="/tmp/scx-flow-v2.3.2-build"
...
# ──────────────────────────────────────────────
echo "============================================================"
echo " Install scx_flow v2.3.2 (Temporal Budget + Adaptive Slice)"
echo "============================================================"
...
# 1. Clone and build
# ──────────────────────────────────────────────
step "Cloning scx_flow v2.3.2 branch"
if [ -d "$BUILD_DIR" ]; then
    rm -rf "$BUILD_DIR"
fi
...
if [ -f "$SYSTEMD_SERVICE" ]; then
    ...
    info "scx.service restarted with v2.3.2 binary."
else
    info "scx.service not found — creating it via install.sh."
    TESTING_CLONE="/tmp/scx-flow-v2.3.2-testing"
    rm -rf "$TESTING_CLONE"
...
echo ""
echo "============================================================"
echo "  scx_flow v2.3.2 installed."
echo "============================================================"
echo ""
echo "  The v2.3.2 binary replaces /usr/bin/scx_flow."
echo ""
echo "  v2.3.2 adds the adaptive minimum slice: budget-exhausted tasks"
