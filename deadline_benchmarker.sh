...
#
# Compare schedulers using a periodic frame-target deadline probe.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BENCHMARK_SCRIPT="$SCRIPT_DIR/deadline_benchmark.sh"
PLOTTER_SCRIPT="$SCRIPT_DIR/deadline_benchmarker_plot.py"
RESET_SCRIPT="$SCRIPT_DIR/reset_sched_ext_state.sh"
RESULTS_ROOT="$SCRIPT_DIR/deadline-comparison-results"
...
say()  { printf "${BOLD}${CYAN}[deadline-mini]${NC} %s\n" "$1"; }
...
usage() {
    cat <<EOF
Usage: ./deadline_benchmarker.sh [options]

Compare schedulers using a periodic frame-target deadline benchmark and generate
CSV/PNG/SVG outputs.
...
Options:
  ...
  --period-us N             Frame target period in microseconds (default: ${PERIOD_US})
  --workers N               Number of deadline probe workers (default: ${WORKERS})
...
run_single_benchmark() {
    ...
    say "Running deadline benchmark for $label"
...
prune_old_results() {
    ...
    warn "Pruning old deadline result directories, keeping the newest ${KEEP_RESULTS}"
...
require_prereqs() {
    [ -x "$BENCHMARK_SCRIPT" ] || {
        err "Missing deadline benchmark script: $BENCHMARK_SCRIPT"
        exit 1
    }
    [ -f "$PLOTTER_SCRIPT" ] || {
...
ok "Deadline benchmark comparison complete"
