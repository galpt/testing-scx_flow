...
#
# Install scx_flow v2.3.5 (temporal budget + affinity-based containment escape)
# from the galpt/scx fork branch scx_flow-v2.3.5.
#
# v2.3.5 fixes the root cause of the Cyberpunk 3-second game freeze: Wine/Proton
# game threads are frequently pinned to specific CPUs.  When such a thread
# exhausts budget and stays runnable during back-to-back dispatches (complex
# game scene), the temporal urgency ratio collapses to 1 and the task enters
# containment with no forward progress.
#
# The fix: tightly-affinitized tasks (nr_cpus_allowed <= 4) are exempt from
# containment.  They stay in the shared lane with a normal slice and can be
# preempted by latency-class tasks.  No forced decay, no adaptive slice, no
# side effects on non-affinitized tasks or audio.
#
# This is based on analysis of scx_lavd's latency-criticality approach, which
# naturally handles pinned/affinitized tasks by boosting their priority.
#
# This installs to /usr/bin/scx_flow (same path as the stable v2.2.6), so
# uninstall.sh can cleanly remove it, and install_scx_flow_standalone.sh can
# replace it with the upstream version.
#
# Usage:
#   sudo ./install_scx_flow_v2.3.5.sh
#
# To switch between versions:
#   sudo ./install_scx_flow_v2.3.0.sh    # temporal budget (game freezes)
#   sudo ./install_scx_flow_v2.3.5.sh    # temporal budget + affinity escape
...
BUILD_DIR="/tmp/scx-flow-v2.3.5-build"
...
# ──────────────────────────────────────────────
echo "============================================================"
echo " Install scx_flow v2.3.5 (Temporal Budget + Affinity Escape)"
echo "============================================================"
...
# 1. Clone and build
# ──────────────────────────────────────────────
step "Cloning scx_flow v2.3.5 branch"
if [ -d "$BUILD_DIR" ]; then
    rm -rf "$BUILD_DIR"
fi
...
if [ -f "$SYSTEMD_SERVICE" ]; then
    ...
    info "scx.service restarted with v2.3.5 binary."
else
    info "scx.service not found — creating it via install.sh."
    TESTING_CLONE="/tmp/scx-flow-v2.3.5-testing"
    rm -rf "$TESTING_CLONE"
...
echo ""
echo "============================================================"
echo "  scx_flow v2.3.5 installed."
echo "============================================================"
echo ""
echo "  The v2.3.5 binary replaces /usr/bin/scx_flow."
echo ""
echo "  v2.3.5 fixes the root cause of the Cyberpunk 3-second freeze."
echo "  Wine/Proton game threads that are pinned to specific CPUs are"
echo "  now exempt from containment.  They stay in the shared lane with"
echo "  a normal slice even during back-to-back dispatches."
echo ""
echo "  To install v2.3.0 instead (temporal budget, game freezes):"
