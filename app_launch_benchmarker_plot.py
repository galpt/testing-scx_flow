...
#
"""Render CSV, PNG, SVG, and Markdown summaries for app-launch benchmark env files."""
...
METRICS = [
    ("APP_LAUNCH_OVER_THRESHOLD_RATIO_PCT", "App Launch Over-Threshold Ratio (%)", "lower"),
    ("APP_LAUNCH_P95_US", "App Launch p95 (us)", "lower"),
    ("APP_LAUNCH_P99_US", "App Launch p99 (us)", "lower"),
    ("APP_LAUNCH_MAX_US", "App Launch Max (us)", "lower"),
]
...
def format_metric_value(metric_key: str, value: float | None) -> str:
    if value is None:
        return "n/a"
    if metric_key == "APP_LAUNCH_OVER_THRESHOLD_RATIO_PCT":
        if value < 0.01:
            return f"{value:.4f}"
        return f"{value:.3f}"
    return f"{value:.2f}"
...
def aggregate(rows: list[dict[str, str]]) -> list[dict[str, object]]:
    ...
    for scheduler, items in grouped.items():
        ...
        entry: dict[str, object] = {
            ...
            "workers": representative.get("APP_LAUNCH_WORKERS", ""),
            "command": representative.get("APP_LAUNCH_COMMAND", ""),
            "late_threshold_us": representative.get("APP_LAUNCH_LATE_THRESHOLD_US", ""),
...
def write_csv(out_dir: Path, aggregated: list[dict[str, object]]) -> Path:
    csv_path = out_dir / "app_launch_benchmarker_summary.csv"
    with csv_path.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow([
            ...
            "command",
            "late_threshold_us",
            "app_launch_over_threshold_ratio_pct",
            "app_launch_p95_us",
            "app_launch_p99_us",
            "app_launch_max_us",
            "notes",
        ...
        for entry in aggregated:
            writer.writerow([
                ...
                "" if entry["APP_LAUNCH_OVER_THRESHOLD_RATIO_PCT"] is None else f"{entry['APP_LAUNCH_OVER_THRESHOLD_RATIO_PCT']:.4f}",
                "" if entry["APP_LAUNCH_P95_US"] is None else f"{entry['APP_LAUNCH_P95_US']:.2f}",
                "" if entry["APP_LAUNCH_P99_US"] is None else f"{entry['APP_LAUNCH_P99_US']:.2f}",
                "" if entry["APP_LAUNCH_MAX_US"] is None else f"{entry['APP_LAUNCH_MAX_US']:.2f}",
                entry["notes"],
                entry["log_paths"],
...
def render_chart(out_dir: Path, aggregated: list[dict[str, object]]) -> tuple[Path, Path]:
    ...
    command = next((str(entry.get("command", "")).strip() for entry in aggregated if str(entry.get("command", "")).strip()), "")
    ...
    if workers:
        subtitle_parts.append(f"Workers: {workers}.")
    if command:
        subtitle_parts.append(f"Command: {command}.")
    if late_threshold_us:
        subtitle_parts.append(f"Soft launch threshold: {late_threshold_us}us.")
    fig.suptitle("App Launch Benchmarker Comparison", fontsize=14, fontweight="bold")
    fig.text(0.5, 0.955, " ".join(subtitle_parts), ha="center", va="top", fontsize=10)
    fig.tight_layout(rect=(0, 0, 1, 0.94))

    png_path = out_dir / "app_launch_benchmarker_comparison.png"
    svg_path = out_dir / "app_launch_benchmarker_comparison.svg"
...
def write_report(out_dir: Path, aggregated: list[dict[str, object]]) -> Path:
    report_path = out_dir / "app_launch_benchmarker_report.md"
    run_count_summary = summarize_run_counts(aggregated)
    workers = next((str(entry.get("workers", "")).strip() for entry in aggregated if str(entry.get("workers", "")).strip()), "")
    command = next((str(entry.get("command", "")).strip() for entry in aggregated if str(entry.get("command", "")).strip()), "")
    late_threshold_us = next((str(entry.get("late_threshold_us", "")).strip() for entry in aggregated if str(entry.get("late_threshold_us", "")).strip()), "")
    ...
    if late_threshold_us:
        lines.append(f"Soft launch threshold: {late_threshold_us}us")
    lines.extend([
        "",
    ...
    for entry in aggregated:
        lines.append(
            "| {scheduler} | {runs} | {status} | {sched_ext_state} | {current_scheduler} | {over_ratio} | {p95} | {p99} | {max_launch} |".format(
                ...
                over_ratio="n/a" if entry["APP_LAUNCH_OVER_THRESHOLD_RATIO_PCT"] is None else f"{entry['APP_LAUNCH_OVER_THRESHOLD_RATIO_PCT']:.4f}",
                p95="n/a" if entry["APP_LAUNCH_P95_US"] is None else f"{entry['APP_LAUNCH_P95_US']:.2f}",
                p99="n/a" if entry["APP_LAUNCH_P99_US"] is None else f"{entry['APP_LAUNCH_P99_US']:.2f}",
                max_launch="n/a" if entry["APP_LAUNCH_MAX_US"] is None else f"{entry['APP_LAUNCH_MAX_US']:.2f}",
            )
        )
    lines.extend([
        "",
