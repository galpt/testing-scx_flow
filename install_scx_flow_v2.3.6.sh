...
#
# Install scx_flow v2.3.6 (temporal budget + affinity escape + dispatch fix)
# from the galpt/scx fork branch scx_flow-v2.3.6.
#
# v2.3.6 fixes two bugs found in v2.3.5 testing:
#
# 1. Dispatch priority inversion (watchdog crash fix):
#    flow_dispatch() checked CONTAINED_DSQ before SHARED_DSQ, causing a
#    contained GameThread to be dispatched ahead of wineserver in the
#    shared lane — resulting in a tight dispatch loop and a 6-second
#    task stall that triggered the sched_ext watchdog.
#
# 2. Containment recurrence escape (game freeze fix):
#    The v2.3.5 tight-affinity escape (nr_cpus_allowed <= 4) only
#    catches pinned game threads.  For unaffinitized game threads,
#    a containment_count counter tracks consecutive containment quanta
#    without sleep.  After 8 consecutive quanta (~400us), the task is
#    force-escaped to the shared lane with a normal slice.
#
# This installs to /usr/bin/scx_flow (same path as the stable v2.2.6),
# so uninstall.sh can cleanly remove it.
#
# Usage:
#   sudo ./install_scx_flow_v2.3.6.sh
#
# To switch between versions:
#   sudo ./install_scx_flow_v2.3.0.sh    # temporal budget (game freezes)
#   sudo ./install_scx_flow_v2.3.6.sh    # v2.3.6 (affinity + dispatch + recurrence)
...
BUILD_DIR="/tmp/scx-flow-v2.3.6-build"
...
# ──────────────────────────────────────────────
echo "============================================================"
echo " Install scx_flow v2.3.6 (Dispatch Fix + Recurrence Escape)"
echo "============================================================"
...
# 1. Clone and build
# ──────────────────────────────────────────────
step "Cloning scx_flow v2.3.6 branch"
if [ -d "$BUILD_DIR" ]; then
    rm -rf "$BUILD_DIR"
fi
...
if [ -f "$SYSTEMD_SERVICE" ]; then
    ...
    info "scx.service restarted with v2.3.6 binary."
else
    info "scx.service not found — creating it via install.sh."
    TESTING_CLONE="/tmp/scx-flow-v2.3.6-testing"
    rm -rf "$TESTING_CLONE"
...
echo ""
echo "============================================================"
echo "  scx_flow v2.3.6 installed."
echo "============================================================"
echo ""
echo "  The v2.3.6 binary replaces /usr/bin/scx_flow."
echo ""
echo "  v2.3.6 fixes two bugs found in v2.3.5:"
echo ""
echo "  1. Dispatch priority inversion (watchdog crash fix):"
echo "     CONTAINED_DSQ was checked before SHARED_DSQ in the dispatch"
echo "     path, causing contained game threads to starve shared-lane"
echo "     tasks indefinitely.  This crashed the scheduler with a"
echo "     runnable task stall (~6s watchdog timeout)."
echo ""
echo "  2. Containment recurrence escape (game freeze fix):"
echo "     Unaffinitized game threads are now force-escaped from"
echo "     containment after 8 consecutive quanta (~400us) without"
echo "     sleeping.  They graduate to the shared lane with a normal"
echo "     slice (900us default)."
echo ""
echo "  To install v2.3.0 (temporal budget, no game freeze fix):"
