...
#
# Usage:
#   sudo ./install_scx_flow_v2.3.4.sh
#
# To switch between versions:
#   sudo ./install_scx_flow_v2.3.0.sh    # temporal budget (no adaptive slice)
#   sudo ./install_scx_flow_v2.3.4.sh    # temporal budget + adaptive slice
...
BUILD_DIR="/tmp/scx-flow-v2.3.4-build"
...
# 1. Clone and build
# ──────────────────────────────────────────────
step "Cloning scx_flow v2.3.4 branch"
if [ -d "$BUILD_DIR" ]; then
    rm -rf "$BUILD_DIR"
fi
...
if [ -f "$SYSTEMD_SERVICE" ]; then
    ...
    info "scx.service restarted with v2.3.4 binary."
else
    info "scx.service not found — creating it via install.sh."
    TESTING_CLONE="/tmp/scx-flow-v2.3.4-testing"
    rm -rf "$TESTING_CLONE"
...
echo ""
echo "============================================================"
echo "  scx_flow v2.3.4 installed."
echo "============================================================"
echo ""
