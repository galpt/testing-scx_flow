...
#
# Report whether scx_flow is installed, configured, and currently active.
...
main() {
    ...
    _sched_ext_state=$(sched_ext_state)
    ...
    if [ -n "$_sched_ext_state" ]; then
        log_info "sched_ext state: ${_sched_ext_state}"
    fi
    ...
    if [ "$_installed" = "yes" ] &&
       ...
       is_active_scheduler_match "$_active"; then
        log_ok "scx_flow is installed, configured, and currently active"
        _rc=0
    elif [ "$_installed" = "yes" ] &&
         [ -n "$_active" ] &&
         is_active_scheduler_match "$_active" &&
         [ "$_sched_ext_state" = "enabled" ]; then
        log_warn "scx_flow is active in sched_ext, but not currently managed by scx.service"
        if [ "$_service_state" = "inactive" ]; then
            log_warn "This usually means a manual benchmark/helper run launched scx_flow directly"
        fi
    else
        log_warn "scx_flow is not fully active right now"
        if [ "$_configured" = "$BINARY_NAME" ] && [ -z "$_active" ]; then
            log_warn "Configured for scx_flow, but no active sched_ext scheduler is reported"
        elif [ "$_configured" = "$BINARY_NAME" ] && [ -n "$_active" ] &&
             ! is_active_scheduler_match "$_active"; then
            log_warn "Configured for scx_flow, but another scheduler is active"
        fi
    fi
