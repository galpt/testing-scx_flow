...
#
# Compare schedulers using an app-launch latency benchmark.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BENCHMARK_SCRIPT="$SCRIPT_DIR/app_launch_benchmark.sh"
PLOTTER_SCRIPT="$SCRIPT_DIR/app_launch_benchmarker_plot.py"
RESET_SCRIPT="$SCRIPT_DIR/reset_sched_ext_state.sh"
RESULTS_ROOT="$SCRIPT_DIR/app-launch-comparison-results"
...
say()  { printf "${BOLD}${CYAN}[app-launch-mini]${NC} %s\n" "$1"; }
...
usage() {
    cat <<EOF
Usage: ./app_launch_benchmarker.sh [options]

Compare schedulers using an app-launch latency benchmark and generate
CSV/PNG/SVG outputs.
...
Options:
  ...
  --workers N               Number of launch workers (default: ${WORKERS})
...
APP_LAUNCH_P99_US=
APP_LAUNCH_MAX_US=
APP_LAUNCH_LATE_THRESHOLD_US=
APP_LAUNCH_OVER_THRESHOLD_COUNT=
...
run_single_benchmark() {
    ...
    say "Running app-launch benchmark for $label"
...
prune_old_results() {
    ...
    warn "Pruning old app-launch result directories, keeping the newest ${KEEP_RESULTS}"
...
require_prereqs() {
    [ -x "$BENCHMARK_SCRIPT" ] || { err "Missing app-launch benchmark script: $BENCHMARK_SCRIPT"; exit 1; }
    [ -f "$PLOTTER_SCRIPT" ] || { err "Missing plotter script: $PLOTTER_SCRIPT"; exit 1; }
    [ -x "$SCRIPT_DIR/app_launch_probe.py" ] || { err "Missing or non-executable app-launch probe helper: $SCRIPT_DIR/app_launch_probe.py"; exit 1; }
...
ok "App-launch benchmark comparison complete"
