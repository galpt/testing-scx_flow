...
if [ -f "$SYSTEMD_SERVICE" ]; then
    ...
    # Clone testing-scx_flow to get install.sh
    TESTING_CLONE="/tmp/scx-flow-v2.3.0-testing"
    rm -rf "$TESTING_CLONE"
    git clone --depth 1 "https://github.com/galpt/testing-scx_flow.git" "$TESTING_CLONE"
    # Run install.sh with the v2.3.0 source
    # (install.sh builds again, but we already have the binary at /usr/bin/scx_flow)
